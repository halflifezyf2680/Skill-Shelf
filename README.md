...
Skill Hub 不做这些事。每个 skill 就是一份自包含的 Markdown 文档，路由只负责一件事：确定性查找。从 230+ 个 skill 的面，精准定位到目标 skill。找到对的文档，读给 LLM 看，结束。

路由层不预设 skill 之间的协作关系，但 LLM 读到多个 skill 后自行判断协作需要——这是 LLM 的主动性，不是基础设施的事。
...
- [superpowers](https://github.com/obra/superpowers)（MIT License）— AI 编程工作流技能框架
